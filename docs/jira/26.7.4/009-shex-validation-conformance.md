...
### Update (2026-07-06): 80/20 ShExC support added
`lib/src/parser/shexc.pest` + `lib/src/shexc_parser.rs` now parse **ShExC** (compact syntax) directly into the same `Schema` AST the ShExJ path builds — no new validator, no duplicated validation logic (`TripleStore::validate_shex_c` parses then delegates to `shex_native::validate_shex_schema`, the same function `validate_shex` uses). Covers `PREFIX`/`BASE`, shape declarations, `CLOSED`/`EXTRA`, triple constraints with cardinality (`*`/`+`/`?`/`{m,n}`), `EachOf` (`;`)/`OneOf` (`|`), `ShapeAnd`/`ShapeOr`/`ShapeNot` (`AND`/`OR`/`NOT`), shape references (`@<label>`), NodeKind keywords, datatype/numeric/string facets, and value sets (IRI/IriStem/language-tagged literals). Verified against 32 of the 33 vendored cases' real original `source_shexc` fields (the one mock-suite case has no such field) — each parsed independently and diffed against its own known-correct `schema.json`, zero mismatches (`lib/tests/shexc_parser.rs`). Out-of-scope constructs (semantic actions, imports, `$label`/`&label` triple-expression labels/inclusion, ShapeMap compact syntax, value-set stem exclusions, annotations) return a clear `Result::Err`, never a silent wrong parse or a panic — verified directly (`out_of_scope_semantic_action_returns_clear_error_not_panic`, `out_of_scope_triple_expr_label_returns_clear_error_not_panic`).
